        ...
        <div class="main-block">
          <el-form ref="searchForm" :model="searchForm" label-width="80px" :inline="true">
            <el-form-item label="标签：" prop="name">
              <el-input class="myInput" v-model="searchForm.name" placeholder="代理标签"></el-input>
            </el-form-item>
            <el-form-item>
...
<script>
  import { agentTag, getagentTag, addagentTag, changeagentTag, deleteagentTag } from '@/api/agentlabel'
  ...
  export default {
    ...
    methods: {
      ...
      handleView(score) {
        ...
        getagentTag({id:score.row.id}).then(res =>{
      ...
      getTagsList(num) {
        ...
        agentTag(this.searchForm).then(res => {
        ...
        this.$refs['userForm'].validate((valid) => {
          if (valid) {
            if(this.dialogType == '新增标签') {
              addagentTag(this.userForm).then(res => {
                if(res.status == 1) {
                  this.$message({
                    type: 'success',
                    message: '新增成功！'
            ...
            if(this.dialogType == '修改标签') {
              changeagentTag(this.userForm).then(res => {
                if(res.status == 1) {
                  this.$message({
                    type: 'success',
                    message: '修改成功！'
      ...
      handleDelete(score) {
        this.$confirm('确定要删除该标签吗？', {
          ...
          .then(async () => {
            await deleteagentTag({id:score.row.id}).then(res => {
                if(res.status == 1){
                    this.getTagsList();
                    this.$message({
                        type: 'success',
