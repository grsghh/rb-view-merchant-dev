...
<script>
  import { refund, getReasonsById, addRefund, editRefund, delRefund,getAllRefund } from '@/api/refundReason'
  ...
  export default {
    ...
    methods: {
      ...
      handleView(score) {
        ...
        getReasonsById(score.row.id).then(res =>{
      ...
      getList(num) {
        ...
        refund(this.searchForm).then(res => {
      ...
      confirmUser(score) {
        this.$refs['userForm'].validate((valid) => {
          if (valid) {
            if(this.dialogType == '新增退款原因') {
              addRefund(this.userForm).then(res => {
                if(res.status == 1) {
                  this.$message({
                    type: 'success',
                    message: '新增成功！'
            ...
            if(this.dialogType == '修改退款原因') {
              editRefund(this.userForm).then(res => {
                if(res.status == 1) {
                  this.$message({
                    type: 'success',
                    message: '修改成功！'
      ...
      handleDelete(score) {
        this.$confirm('确定要删除该备注吗？', {
          ...
          .then(async () => {
            await delRefund(score.row.id).then(res => {
              if(res.status == 1){
                this.$message({
                  type: 'success',
                  message: '删除成功!'
