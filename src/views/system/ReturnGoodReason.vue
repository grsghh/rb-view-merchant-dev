...
<script>
  import {
      returnList,
      getReturnReasonsById,
      addReturn,
      editReturn,
      delReturn,
      getAllReturn
  } from '@/api/refundReason'
  import Pagination from '@/components/Pagination'
  export default {
    name: 'ReturnGoodReason',
    ...
    methods: {
      ...
      handleView(score) {
        ...
        getReturnReasonsById(score.row.id).then(res =>{
      ...
      getList(num) {
        ...
        returnList(this.searchForm).then(res => {
      ...
      confirmUser(score) {
        this.$refs['userForm'].validate((valid) => {
          if (valid) {
            if(this.dialogType == '新增退货原因') {
              addReturn(this.userForm).then(res => {
                if(res.status == 1) {
                  this.$message({
                    type: 'success',
                    message: '新增成功！'
            ...
            if(this.dialogType == '修改退货原因') {
              editReturn(this.userForm).then(res => {
                if(res.status == 1) {
                  this.$message({
                    type: 'success',
                    message: '修改成功！'
      ...
      handleDelete(score) {
        this.$confirm('确定要删除该吗？', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
          type: 'warning'
        })
          .then(async () => {
            await delReturn(score.row.id).then(res => {
              if(res.status == 1){
                this.$message({
                  type: 'success',
                  message: '删除成功!'
